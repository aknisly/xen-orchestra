...
Create a consolidate log from individual event logs.

It can be used directly as an `onProgress` callback:

```js
import { makeOnProgress } from '@vates/task/combineLogs'

const onProgress = makeOnProgress()

Task.run({ name: 'my task', onProgress }, asyncFn)

// the last state of the task is always available at
onProgress.task

// to wait for the end of task:
await onProgress.getResult()
```
